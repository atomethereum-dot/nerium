...
(function(){
  ...
  function dibuja(p){
    ...
    /* EL CANTO DE LA ABERTURA, ENCENDIDO. Sin el, lo que crece es un agujero:
       se ve la pagina de detras pero no se ve que algo se ESTA abriendo. Un
       filo de luz siguiendo el contorno del glifo mientras crece -y solo
       mientras crece- convierte el agujero en una puerta. Va con «stroke», que
       sigue la misma ruta del texto que se acaba de recortar, asi que no hay
       manera de que se despegue del hueco. */
    var filo=Math.sin(Math.PI*Math.min(1,t*1.9));
    if(filo>0.01){
      cx.globalAlpha=filo*0.9*tinta;
      cx.lineWidth=Math.max(0.6,2.2/esc);
      cx.strokeStyle='rgba(206,226,255,1)';
      cx.shadowColor='rgba(120,170,255,.95)';
      cx.shadowBlur=26/esc;
      cx.strokeText(cifraTxt,W/2,cifraY);
      cx.shadowBlur=0;
      cx.globalAlpha=1;
    }
    cx.restore();
