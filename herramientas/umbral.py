...
(function(){
  ...
  var tes=[], TLADO=0, cifraTxt='85%', cifraY=0, ojoX=0, ojoY=0, cifraBajo=0, cifraArriba=0;
  ...
  function teselas(txt,tam){
    ...
    cifraBajo=y1+TLADO; cifraArriba=y0;
  ...
  function ren(txt,x,y,tam,alfa,al,col,esp){
  ...
  function marco(p,vel){
    ...
    var bx=Math.min(W*0.86,tamCifra*3.1), x0=(W-bx)/2, x1=x0+bx;
    var by=barraY();
    ...
    if(a1>0.004&&rot.arriba){
      ...
      cx.restore();
      var hueco=tk*1.15;
      ren(rot.arriba, W/2+hueco/2, yr, tk, a1, 'center', 'rgba(228,234,243,.92)', 0.26);
    ...
    ren(rot.dinero, W/2, cifraBajo+tk*2.9, tk*1.18, a3, 'center', 'rgba(236,243,255,.94)', 0.14);

    /* 4 · y las condiciones, al pie */
    var a4=tramo(p,0.30,0.42)*vel;
    ren(rot.pie, W/2, Math.min(H-tk*1.6, ye+tk*2.4), tk*0.84, a4,
        'center', 'rgba(140,166,214,.70)', 0.20);
  }
