...
(function(){
  ...
  /* Acaba en 0,24 y no en 0,30: la primera losa arranca en 0,36 y con la
     ventana anterior las ultimas teselas seguian aterrizando cuando la tinta
     ya se estaba abriendo. Dos cosas a la vez y ninguna se lee. */
  ...
  function pintaCifra(p){
    ...
    var lado=Math.max(2,TLADO-1.1);            /* el hueco de 1,1 px, como la rueda */
    for(var i=0;i<tes.length;i++){
      var t=tes[i];
      ...
      if(k<1){
        ...
        cc.fillStyle=t.c; cc.fillRect(x,y,lado,lado);
        cc.fillStyle='rgba(226,238,255,.55)';
  ...
  function dibuja(p){
    ...
    var alto=H/LOSAS;
    /* la cifra se pinta UNA vez por cuadro en su propio lienzo, y cada losa
       reparte su tajada con el mismo desplazamiento que la tinta: asi cuando
       la plancha se desmonta, la cifra se rasga con ella */
    pintaCifra(p);
